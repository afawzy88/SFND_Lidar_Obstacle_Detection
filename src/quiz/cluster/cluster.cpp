...
void Proximity(int index, const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& isProcessed, KdTree* tree, float distanceTol)
{
	isProcessed[index] = true;

	cluster.push_back(index);

	std::vector<int> nearbyIDs = tree->search(points[index], distanceTol);

	for (int id : nearbyIDs)
	{
		if(!isProcessed[id])
			Proximity(id, points, cluster, isProcessed, tree, distanceTol);
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> points, KdTree* tree, float distanceTol)
{
	...
	while (i < points.size())
	{
		...
		Proximity(i, points, cluster, isProcessed, tree, distanceTol);
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	std::vector<int> nearby = tree->search({-6,6.5},3.0);
